...
## Описание
Веб-приложение с рассылкой сообщений, которое позволяет пользователям управлять рассылками сообщений для клиентов. 
...
*   Создано новое Django-приложение под названием `mail` с помощью команды `python manage.py startapp mail`.
*   Приложение `mail` зарегистрировано в настройках проекта (в `INSTALLED_APPS` в файле `config/settings.py`). 
*   Настроена маршрутизация для приложения:
    *   Создан файл `urls.py` в директории приложения `mail`.
    *   В главном файле `BASE_DIR/urls.py` проекта добавлено подключение маршрутов приложения `mail` с использованием функции `path` и `include`.

**Создание шаблонов:**

*   Создана директория `templates` внутри директории приложения `mail`. 
...
*   В файле `views.py` приложения `mail` созданы два контроллера:
...
*   В файле `urls.py` приложения `mail` настроена маршрутизация для контроллеров с использованием функции `path`:
